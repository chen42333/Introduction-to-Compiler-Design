...
IfNode::IfNode(const uint32_t line, const uint32_t col, ExpressionNode* expr, CompoundStatementNode* then_act, CompoundStatementNode* else_act,Type t)
    : AstNode{line, col, t}, expr(expr) {
        acts.push_back(then_act);
        if(else_act != NULL) acts.push_back(else_act);
    }
...
void IfNode::visitChildNodes(AstNodeVisitor &p_visitor) {
    ...
    for(auto i: acts) i->accept(p_visitor);
}